...
{
  bool ChanNoteMatch = GateChannels[i] == Channel && GateNotes[i] == NoteNumber;
  bool TrigMatch = GateStates[i] == PERCTRIGGER && ChanNoteMatch;
  bool GateMatch = GateStates[i] == PERCGATE && ChanNoteMatch;

  
  typedef void (*f)(int8_t i, int8_t Channel, int8_t NoteNumber, int8_t VelocityValue);
  f router[2] {&play_perc, &play_gate};
  
  if ( GateChannels[i] == Channel && GateNotes[i] == NoteNumber && GateStates[i] == PERCTRIGGER ) {
...
//    MayPlayCvatRow[i] = true;
  ...
  } else if ( GateChannels[i] == Channel && GateNotes[i] == NoteNumber && GateStates[i] == PERCGATE ) {
    digitalWrite(LOGIC_OUTPUTS[i+2], HIGH);
