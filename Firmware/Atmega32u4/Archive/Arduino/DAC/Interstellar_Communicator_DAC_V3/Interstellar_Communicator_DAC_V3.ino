...
//void note_toggle(byte channel, byte note, byte velocity, byte type)
//{
//  byte lastnote;
//  for(byte i = 0; i < 4; i++){
//    for(byte n = 0; n < 20; n++){
...
//          break;
//        }
//        else if(notelist[i][n+1]){ // Shifts all notes to the left where the note was removed.
...
void noteon(byte channel, byte note, byte velocity) //Moeten bytes zijn, geen int. (met int wordt de data corrupt).
{
  ...
  if(chanRange){ //Zorgt ervoor dat de notelist niet gevuld kan worden met waardes buiten de omvang van de list in geval er corrupte midi data wordt ontvangen.
    ...
    static int CVisDrumVel[4];
    static byte GateisTrig = 1;
    static byte Poly_type[4][17];
    static bool Poly_count[4][17];
    static byte Last_poly[4][17];
    byte CVisVel = 2;
    for(byte i = 0; i < 4; i++){
      if(knob_states[i]){
//        if(Poly_count[i][channel] == false){
//          Poly_count[i][channel] = true;
//        }
        if(learn_loop_count == 0){
          ...
          else if(learn_notes_count == 1){ // Drum/pad/key Trigger.
            Gate_channels[i] = channel;
            Gate_notes[i] = note;
            Gate_states[i] = GateisTrig;

            CVisDrumVel[i] = "Drum" + i;
            CV_channels[i] = channel;
            CV_states[i] = CVisDrumVel[i];
          }
        }
