...
void ReadMidi_and_if_True_StartTimer()
{
  if(MIDI.read()){
    ...
    CheckforStartSpiTransaction();
  }
}

void if_LastMidiLongAgo_StopClock_and_TurnNotesOff()
{
  int16_t TimeDifference = millis() - timerMIDIrecieved;
  if( TimeDifference > 500 ){
    timerMIDIrecieved = millis();
    RecievedStop();
    CheckforEndSpiTransaction();
  }
}

void CheckforStartSpiTransaction()
{
  if ( SpiTransactionEnded ) {
    SPI.beginTransaction(SPISettings(8000000, MSBFIRST, SPI_MODE0));
    SpiTransactionEnded = 0;
    Serial.println("SPI started");
    Serial.flush();
  }
}

void CheckforEndSpiTransaction()
{
  if ( SpiTransactionEnded ) {
    SPI.endTransaction();
    SpiTransactionEnded = 1;
