...
IPV van aantal kanalen in Polytype opslaan, tel alle kanalen bij elkaar op. En trek tijdens het spelen het kanaalnummer van Polytype af.
*/
...
void checkpolynoteon()
{
  ...
  for (byte i = 0; i < 4; i++) {
    if (CV_states[i] == CVisKey) {
      if ( PolyNoteonCount[CV_channels[i]] == 0 ) PolyNoteonCount[CV_channels[i]] = i + 1;
...
void Check_and_PlayNotes(uint8_t channel, uint8_t note, uint8_t velocity)
{
  ...
  for(uint8_t i = 0; i < 4; i++){
    ...
    if (CV_channels[i] == channel) {
      Serial.println(PolyNoteonCount[channel]);
      if (PolyNoteonCount[channel] - 1 == i &&
      CV_states[i] == CVisKey) {
        for (uint8_t n = 0; n < LengthNotelist; n++) {
          if (notelist[i][n] == -1) {
           notelist[i][n] = note;
...
//      }
  ...
  if (PolyNoteonCount[channel] < PolyNoteonType[channel]) {
    PolyNoteonCount[channel]++;
  }
  if (PolyVelocityCount[channel] < PolyVelocityType[channel]) {
    PolyVelocityCount[channel]++;
  }
  if (PolyAftertouchCount[channel] < PolyAftertouchType[channel]) {
    PolyAftertouchCount[channel]++;
  }
