...
int8_t PolyNotes[10];
uint8_t LengthPolyNotes;
int8_t notelist[4][10]; //Alle noten hoger dan -1 in deze list zijn ingedrukt.
...
void setnotelists()
{
  ...
  LengthPolyNotes = sizeof(PolyNotes);
  for ( uint8_t n = 0; n < LengthPolyNotes; n++ ) {
    PolyNotes[n] = -1;
  }
}
...
//    PolyNoteonCount[i][j] = 0;
  ...
  for (byte i = 0; i < 4; i++) {
    if (CV_states[i] == CVisKey) {
      ...
      Serial.println(ChannelIsPoly[CV_channels[i]]);
...
{
  if ( LearnMode ) LearnNoteOn(channel, note, velocity);
  else Check_and_PlayNotes(channel, note, velocity);
}
...
{
  Serial.println("Ch:"+String(channel)+" On:"+String(note)+" Vel:"+String(velocity));
  MayPlayNote = true;
  for(uint8_t i = 0; i < 4; i++){
    ...
    if (CV_channels[i] == channel) {
      if ( CV_states[i] == CVisKey ) {
        if ( ChannelIsPoly[channel] && MayPlayNote ) {
          Serial.print(String(i)+" ");
          if ( CurrentNotePlayed[i] == -1 ) {
            PlayNote(note, i);
            CurrentNotePlayed[i] = note;
            MayPlayNote = false;
          } else if ( i == ChannelIsPoly[channel] ) {
...
{
  if ( !LearnMode ) CheckNoteoff(channel, note, velocity);
}

void CheckNoteoff(uint8_t channel, uint8_t note, uint8_t velocity)
{
  Serial.println("Ch:"+String(channel)+" Off:"+String(note)+" Vel:"+String(velocity));
  MayPlayNote = true;
  for (uint8_t i = 0; i < 4; i++) {
          ...
          || notelist[i][n] == -1) {
            if (n < LengthNotelist - 1) {             
              if(notelist[i][n + 1] >= 0){  // Shifts all notes to the left where the note was removed.
                ...
                notelist[i][n] = -1;
                if(n == 0) {
                  LastNote = -1; // If the current note is the first in the array. Then there are no notes on.
                } else {
                  LastNote = notelist[i][n-1]; //  When the next note in the array is also -1, the last note will be the note before the current one.
                }
                break;
              }
...
void FillPolyNotes(uint8_t note)
{
  Serial.print(" Poly: ");
  for ( uint8_t n = 0; n < LengthPolyNotes; n++ ) {
    if ( PolyNotes[n] == -1 ) {
...
void PlayNote(uint8_t note, uint8_t i)
{
  Serial.print("Play:"+String(note));
  if (i < 2) {
    setVoltage(DAC1, i, 1, note<<5);
  } else {
...
void FindNoteInPolyNotes(uint8_t note)
{
  for ( uint8_t n = 0; n < LengthPolyNotes; n++ ) {
    if ( PolyNotes[n] == note ) {
      ShiftPolyNotes(n);
...
{
  Serial.print(" Poly: ");
  for ( n; n < LengthPolyNotes; n++ ) {
    if ( (n == LengthPolyNotes - 1) ) {
      PolyNotes[n] = -1;
