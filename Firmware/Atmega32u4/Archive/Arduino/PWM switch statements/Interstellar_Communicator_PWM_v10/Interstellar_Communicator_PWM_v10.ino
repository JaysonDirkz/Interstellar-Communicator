...
Nu gedaan:
- MIDI thru off weer uitgesckakelt. (dus midi thru staat nu wel aan).
- MIDI debug output in if statement opgenomen zodat het makkelijk aan en uit kan worden gezet.
...
// Beslis tussen output inleren of spelen.
  if ( LearnMode ) {
...
// Deze funtie weghalen. router[i](pwmVal) hernoemen naar pwm_out[i](pwmVal) etc.
void pwm_out(uint8_t i, uint8_t pwmVal)
{
  ...
  if (MIDI_DEBUG) MIDI.sendControlChange(33, pwmVal % 128, i+1);
}
  void pwm_out_6(uint8_t pwmVal) {PWM6 = pwmVal;}
  void pwm_out_9(uint8_t pwmVal) {PWM9 = pwmVal;}
  void pwm_out_11(uint8_t pwmVal) {PWM10 = pwmVal;}
  void pwm_out_13(uint8_t pwmVal) {PWM13 = pwmVal;}
