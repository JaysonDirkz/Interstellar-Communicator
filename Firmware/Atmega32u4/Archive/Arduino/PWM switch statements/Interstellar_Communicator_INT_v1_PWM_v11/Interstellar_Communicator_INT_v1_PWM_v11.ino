...
void play_poly_cv(int8_t i, int8_t Channel, int8_t NoteNumber, int8_t VelocityValue)
{
  switch ( CvStates[i] ) {
    ...
    //      MIDI.sendNoteOn(i, 0, i);
      ...
      break;
    case VELOCITY:
      CurrentVelocityPlayed[i] == NOTHING ? CheckVelocity(i, VelocityValue) : break;
      break;
    case AFTERTOUCH:
      CurrentAftertouchPlayed[i] == NOTHING ? CheckAftertouch(i, NoteNumber) : break;
      break;
    default: break;
  }
}
...
{
  LearnMode ? return : CheckNoteoff(Channel, NoteNumber, VelocityValue);
}
