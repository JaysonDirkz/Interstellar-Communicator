    ...
    if ( MIDI.read() ) {
      for ( int row = 0; row < 4; row++ ) {
        if ( get_buttonStates() & (1 << row) ) {
          int savedCV[row] = { MIDI.getType(), filterData1(), filterData2(), MIDI.getChannel() };
          int savedTG[row] = { MIDI.getType(), filterData1(), filterData2(), MIDI.getChannel() };
...
void play_poly_cv(int8_t i, int8_t Channel, int8_t NoteNumber, int8_t VelocityValue)
{
  switch ( CvStates[i] ) {
    ...
    //      MIDI.sendNoteOn(i, 0, i);
      ...
      break;
    case VELOCITY:
      if (CurrentVelocityPlayed[i] == NOTHING) CheckVelocity(i, VelocityValue);
      break;
    case AFTERTOUCH:
      if (CurrentAftertouchPlayed[i] == NOTHING) CheckAftertouch(i, NoteNumber);
      break;
    default: break;
  }
}
