...
TODO:
- POlyphony alleen naar de volgende output wanneer die vrij is. Niet vrij? Dan laats gebruikte output gebruiken.
...
const int CV_RSHIFT[6] = {-1, 0, 0, 0, 6, 6};
//Saved in eeprom
//------
TYPE cv_type[4];
int8_t cv_channel[4];
...
int8_t polyCounter[16] = {0};

void load_learn_status()
{
    for ( int channels = 0; channels < 16; channels++ ) 
    {
        polyCounter[channels] = 0;
		note_row_hist[channels] = TYPE::NEG;
        polyphony[channels] = 0;
	}
   
	vel_row_hist = TYPE::NEG;
	at_row_hist = TYPE::NEG;
    nrpn_iter[0] = 0;
	nrpn_iter[1] = 0;
	nrpn_iter[2] = 0;
	nrpn_iter[3] = 0;
...
void controlThread()
{
    ...
    if ( currentMillis != lastMillis )
    {
        ...
        {
            case -1: break;
            case 0: digitalWrite(CLOCK, LOW); break;
        ...
        {
            case -1: break;
            case 0: digitalWrite(RESET, LOW); break;
...
void learn_note_on_vel(int8_t channel, int8_t note, int8_t velocity)
{
	...
	for ( int row = 0; row < 4; row++ )
	{
		switch ( (channel == 10) << 1 | (~PINF & rowTo_32u4PINF_bit[row]) > 0 )
		{
    		case KEYS:
    			switch ( count_rows )
    			{
        			case 0:
        				cv_type[row] = TYPE::NOTE;
        				cv_channel[row] = channel;
        				++count_rows;
        				break;
        			case 1:
        				cv_type[row] = TYPE::VELOCITY;
        				cv_channel[row] = channel;
        				++count_rows;
        				break;
        			case 2:
        				cv_type[row] = TYPE::PENDING_AT;
        				cv_channel[row] = channel;
        				break;
        			default:
        				break;
	...
	int8_t count_keys = 0;
	for ( int row = 0; row < 4; row++ ) {
		if ( cv_type[row] == TYPE::NOTE && cv_channel[row] == channel ) {
			++count_keys;
		}
	}
	polyphony[channel - 1] = count_keys;
}
...
void learn_atc(int8_t channel, int8_t aftertouch)
{
	for ( int row = 0; row < 4; row++ )
	{
		if ( cv_channel[row] == channel && cv_type[row] == TYPE::PENDING_AT && (~PINF & rowTo_32u4PINF_bit[row]) > 0 )
		{
			cv_type[row] = TYPE::ATC;
		}
	}
}
...
{
    bool polyOn = polyphony[channel - 1] > 1;
	...
	for ( int row = 0; row < 4; row++ )
	{
		...
		switch ( polyOn << 1 | cv_main )
		{
			...
			case POLY_KEYS:
                ...
                bool nGateState = ! digitalRead(GATES[row]);
                if ( nGateState || (polyphony[channel - 1] == polyCounter[channel - 1] && note_row_hist[channel - 1] == row) )
                {
                    polyCounter[channel - 1] += nGateState; //bij noteOff decrement doen.
                    cv_out(row, note, type);
                    gate_out(row, true);
                    // note_hist_2[row] = note_hist_1[row];
                    note_hist[row] = note;
...
void note_off(int8_t channel, int8_t note)
{
    ...
    // MIDI.sendControlChange(note, perc_main, row+1);
		if ( note_hist[row] == note & (cv_main | perc_main) ) {
			gate_out(row, false);
			note_hist[row] = TYPE::NEG;

            if ( polyOn && polyCounter[channel - 1] > 0)
            {
                polyCounter[channel - 1]--;
            }
...
void off_vel(int8_t channel, int8_t note, int8_t velocity)
{
    ...
    bool cv_main = cv_main_conditions(row, type, channel);
		bool perc_main = perc_main_conditions(row, TYPE::NOTE, channel, note);
...
void atp(int8_t channel, int8_t note, int8_t aftertouch)
{
	...
	int row = 2;
	do {
		switch ( (note_hist[row - 2] == note) << 2 | polyOn << 1 | cv_main_conditions(row, type, channel) )
		{
			case MONO_ATP:
				cv_out(row, aftertouch, type);
				goto END_LOOP;
...
bool cv_main_conditions(int row, int type, int8_t channel)
{
	return
	cv_type[row] == type &
	cv_channel[row] == channel;
...
bool perc_main_conditions(int row, int type, int8_t channel, int8_t number)
{
	return
	perc_type[row] == type &
	perc_channel[row] == channel &
...
void cv_out(int row, int number, int type)
{
	...
	switch (row)
	{
	 ...
	 if ( !MIDI.getThruState() ) {
	 	MIDI.sendControlChange(0, value >> 7, row);
	 	MIDI.sendControlChange(32, value & 127, row);
	 }
}
