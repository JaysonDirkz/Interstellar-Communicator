...
void loop()
{
	if ( MIDI.read() )
    {
        switch ( MIDI.getType() )
        {
            case NoteOn: n_on_callbacks[MIDI.getChannel() - 1]; break;
            case NoteOff: n_off_callback[Type::KeysOff][MIDI.getChannel() - 1]; break;
            case AfterTouchPoly: atp_callbacks[Type::KeysOff][MIDI.getChannel() - 1]; break;
            case AfterTouchChannel: atc_callbacks[Type::KeysOff][MIDI.getChannel() - 1]; break;
            case PitchBend: pb_callbacks[Type::KeysOff][MIDI.getChannel() - 1]; break;
            case ControlChange: cc_callbacks[Type::KeysOff][MIDI.getChannel() - 1]; break;
    ...
    controlThread(); // Set the outputs here (in a for loop).
    setupLearn();
}
...
//            for ( int8_t type = 0; type < (int8_t)MessageType::Size; ++type )
//            {
...
//                        MIDI.sendNoteOn(keysActive[row].getPosition(note), 29, 5);
            
                        velocitySave = lastNote[row].getVelocity();

                        int8_t positionsOfKey = lastNote[row].getPitch();
//                        MIDI.sendNoteOn(positionsOfKey, 30, 5);
//                        MIDI.sendNoteOn(keysActive[row].getKey(positionsOfKey), 31, 5);
...
//                if ( atpAddr > -1 ) cv_out(row, pressures[atpAddr][lastKey], 0);
...
//        MIDI.sendNoteOn(polyAddr, 0, 5); //debug
        ...
        if ( polyAddr > -1 )
        {
//            pressures[polyToPressureMap[polyAddr]].set(note, aftertouch);
...
//                if ( c == polyphony.counter[polyAddr] )
//                {
//                    row = polyphony.rowAtCount[polyAddr][c];
//                    break;
//                }
...
//    MessageType type = MessageType::ControlChange;
